...
use App\DataTables\RapportFinalDataTable;
...
class RapportFinalController extends AppBaseController
{
    /** @var  DepotRepository */
    ...
    public function __construct(DepotRepository $depotRepo)
    {
        ...
        $this->middleware('etudiant')->only([ 'create']);
    }
     ...
     * @return Response
     */
    public function index(RapportFinalDataTable $depotDataTable)
    {
        return $depotDataTable->render('rapports_finaux.index');
    }
     ...
     * @return Response
     */
    public function create(\App\Repositories\StageRepository $stageRepo)
    {
        return view('rapports_finaux.create')->withStage(
                    $stageRepo->getLastStage(Auth::id())
                );
    }
    ...
    {
        $inputs = $request->all();
        $message = 'Rapport déposé avec succès.Veuillez attendre la valdiation de vôtre professeur'
                . ' de suivi ou Directeur de mémoire';
        $inputs['path'] = Util::uploadDocument($request->file('path'));
        $inputs['final'] = true;
        Depot::where('stage_id', $inputs['stage_id'])->where('final', true)->delete();
    ...
    public function validateDepot(Depot $depot) {
        ...
        $stage->save();
        return HttpUtil::sendSuccessDialogResponse('Rapport validé avec succès');
    }
